...
export class BogoSort implements Algorithm {
  init(entryNode: INode): Snapshot[] {
    ...
    while (!isSorted() && iterations < MAX_ITERATIONS) {
      ...
      // Algoritmo Fisher-Yates paso a paso
      for (let i = n - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        
        // Solo guardamos snapshot si realmente intercambia dos nodos distintos
        if (i !== j) {
          [values[i], values[j]] = [values[j], values[i]];
          [order[i], order[j]] = [order[j], order[i]];
          
          // Ahora pasamos exactamente dos IDs a la tupla de swapping
          snapshots.push({
            ...snap(),
            highlights: { swapping: [order[i], order[j]] } 
          });
        }
      }
